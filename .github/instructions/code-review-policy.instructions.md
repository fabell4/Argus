...
- Both processes share `requirements.txt`; audit covers both the scheduler and API processes

### Additional Security Checks

- NUT socket connections must specify a connect timeout; community strings and credentials must
...
- `data/runtime_config.json` writes must use the atomic write pattern at all call sites
- NUT and SNMP pollers must catch `socket.timeout` and `OSError` and perform exactly one retry
  before marking a device as offline
